...
The default stack is **Ghostty** (outer terminal) → **Starship** (prompt), with no automatically launched multiplexer. **Herdr is installed and supported as an optional multiplexer** in Ghostty; its chezmoi-managed config uses `ctrl+space` as the prefix. Zellij was fully removed on 2026-06-13 (brew formula, `~/.config/zellij/`, the `zcd` alias, and the zellaude hooks in `~/.claude/settings.json`) — treat any `dot_config/zellij/` reference as dead; it was **not** revived by the tmux/workmux addition below (that's a separate, tmux-based path). **WezTerm was removed on 2026-06-26** (app uninstalled). Its config was *not* deleted — it lives at `archive/wezterm/wezterm.lua`, which the `archive` entry in `.chezmoiignore` keeps out of the home dir. Treat any `dot_config/wezterm/` reference as dead. **Verify the live stack before acting on it** — `which ghostty wezterm herdr`, `chezmoi managed | grep -E 'ghostty|wezterm|herdr'`.
...
- Herdr config: `dot_config/herdr/config.toml` — optional multiplexer, stable update channel, `ctrl+space` prefix, Kohra palette. Still the daily-driver multiplexer for regular single-session work.
...
- **tmux + workmux (added 2026-07-03)**: a second, narrower-purpose multiplexer path used only for parallel Claude Code sessions across git worktrees — `dot_tmux.conf` (Claude Code terminal passthrough/extended-keys settings, plus `prefix+Ctrl-s` workmux dashboard popup, `prefix+Ctrl-t` sidebar toggle) and `dot_config/workmux/config.yaml` (global workmux defaults: `agent: claude`, nerdfont icons, left sidebar). Entered via the `wm` zsh alias (attaches/creates a tmux session named `work`); `workmux add <branch>` spins up a worktree + tmux window running Claude, `workmux merge`/`workmux remove` clean up. This is **not** a replacement for Herdr — Herdr stays the default for regular terminal use. `workmux setup` also installed Claude Code status-tracking hooks in `~/.claude/settings.json` and the `/rebase`, `/worktree`, `/coordinator`, `/open-pr`, `/workmux` skills; workmux's own bundled `/merge` skill was intentionally **not** installed because it collides by name with this repo owner's existing `/merge` skill (`~/dev/agent-config/skills/merge`, for landing GitHub PRs) — use `workmux merge` (the CLI command) directly instead of a `/merge` slash command when finishing a worktree branch. Zellij backend was considered and rejected: workmux's Zellij support requires building Zellij from source (targets unreleased post-0.43 features) and is missing dashboard previews, status icons, and session mode compared to the tmux backend.
